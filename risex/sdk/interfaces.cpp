...
IDirect3DDevice9 * Interfaces::m_device() {
	if (!p_device)
		p_device = **reinterpret_cast<IDirect3DDevice9***>(util::FindSignature(crypt_str("shaderapidx9.dll"), globals.signatures.at(0).c_str()) + 0x1);

	return p_device;
}

IVEngineClient * Interfaces::m_engine() {
	if (!p_engine)
...
IInputSystem * Interfaces::m_inputsys() {
	if (!p_inputsys)
...
IBaseClientDLL * Interfaces::m_client() {
	if (!p_client)
...
IClientEntityList * Interfaces::m_entitylist() {
	if (!p_entitylist)
...
IEngineTrace * Interfaces::m_trace() {
	if (!p_trace)
...
IEngineSound* Interfaces::m_enginesound() {
	if (!p_enginesound)
...
DWORD* Interfaces::m_inputinternal() {
	if (!p_inputinternal)
...
IVRenderView * Interfaces::m_renderview() {
	if (!p_renderview)
...
IMDLCache * Interfaces::m_modelcache() {
	if (!p_modelcache)
...
IVModelRender * Interfaces::m_modelrender() {
	if (!p_modelrender)
...
IMaterialSystem * Interfaces::m_materialsystem() {
	if (!p_materialsystem)
...
IVModelInfoClient * Interfaces::m_modelinfo() {
	if (!p_modelinfo)
...
IPrediction * Interfaces::m_prediction() {
	if (!p_prediciton)
...
IGameMovement * Interfaces::m_gamemovement() {
	if (!p_gamemovement)
...
IVDebugOverlay * Interfaces::m_debugoverlay() {
	if (!p_debugoverlay)
...
IPhysicsSurfaceProps * Interfaces::m_physsurface() {
	if (!p_physsurface)
...
IGameEventManager2 * Interfaces::m_eventmanager() {
	if (!p_eventmanager)
...
IViewRenderBeams * Interfaces::m_viewrenderbeams() {
	if (!p_viewrenderbeams)
		p_viewrenderbeams = *reinterpret_cast<IViewRenderBeams **>(util::FindSignature(crypt_str("client.dll"), globals.signatures.at(1).c_str()) + 0x1);

	return p_viewrenderbeams;
}

IMemAlloc * Interfaces::m_memalloc() {
	if (!p_memalloc)
...
IClientMode * Interfaces::m_clientmode() {
	if (!p_clientmode)
...
CGlobalVarsBase * Interfaces::m_globals() {
	if (!p_globals)
...
CGlowObjectManager * Interfaces::m_glow() {
	if (!p_glow)
		p_glow = *reinterpret_cast<CGlowObjectManager**>(util::FindSignature(crypt_str("client.dll"), globals.signatures.at(2).c_str()) + 0x3);

	return p_glow;
}

CClientState * Interfaces::m_clientstate() {
	if (!p_clientstate)
...
IMoveHelper * Interfaces::m_movehelper() {
	if (!p_movehelper)
		p_movehelper = **reinterpret_cast<IMoveHelper***>(util::FindSignature(crypt_str("client.dll"), globals.signatures.at(3).c_str()) + 0x2);

	return p_movehelper;
}

CInput * Interfaces::m_input() {
	if (!p_input)
		p_input = *reinterpret_cast<CInput**>(util::FindSignature(crypt_str("client.dll"), globals.signatures.at(4).c_str()) + 0x1);

	return p_input;
}

C_CSPlayerResource * Interfaces::m_playerresource() {
	if (!p_playerresource)
		p_playerresource = **reinterpret_cast<C_CSPlayerResource ***>(util::FindSignature(crypt_str("client.dll"), globals.signatures.at(5).c_str()) + 0x2);
	else if (globals.g.should_update_playerresource)
	{
		globals.g.should_update_playerresource = false;
		p_playerresource = **reinterpret_cast<C_CSPlayerResource ***>(util::FindSignature(crypt_str("client.dll"), globals.signatures.at(5).c_str()) + 0x2);
	}
...
CSGameRulesProxy * Interfaces::m_gamerules() {
	if (!p_gamerules)
		p_gamerules = **reinterpret_cast<CSGameRulesProxy ***>(util::FindSignature(crypt_str("client.dll"), globals.signatures.at(6).c_str()) + 0x1);
	else if (globals.g.should_update_gamerules)
	{
		globals.g.should_update_gamerules = false;
		p_gamerules = **reinterpret_cast<CSGameRulesProxy ***>(util::FindSignature(crypt_str("client.dll"), globals.signatures.at(6).c_str()) + 0x1);
	}
...
IBaseFileSystem* Interfaces::m_basefilesys() {
	if (!p_basefilesys)
...
ILocalize * Interfaces::m_localize() 
{
	if (!p_localize)
...
DWORD Interfaces::m_postprocessing() {
	if (!p_postprocessing)
		p_postprocessing = *(DWORD*)(util::FindSignature(crypt_str("client.dll"), globals.signatures.at(7).c_str()) + 0x2);
	
	return p_postprocessing;
}

DWORD Interfaces::m_ccsplayerrenderablevftable() {
	if (!p_ccsplayerrenderablevftable)
		p_ccsplayerrenderablevftable = *(DWORD *)(util::FindSignature(crypt_str("client.dll"), globals.signatures.at(8).c_str()) + 0x4E);

	return p_ccsplayerrenderablevftable;
}

IWeaponSystem* Interfaces::m_weaponsys() {

	if (!p_weaponsys)
		p_weaponsys = *reinterpret_cast<IWeaponSystem**>(util::FindSignature(crypt_str("client.dll"), globals.signatures.at(21).c_str()) + 2);
