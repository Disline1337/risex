...
#include "..\lagcompensation\animfix.h"
...
public:
	...
	scan_point(const Vector& point, const int& hitbox, const bool& center) //-V818
	{
		this->point = point;
		this->hitbox = hitbox;
		this->center = center;
...
public:
	...
	void scan(adjust_data* record, scan_data& data, const Vector& shoot_position = globals.g.eye_pos);
	std::vector <int> get_hitboxes(adjust_data* record);
	std::vector<scan_point> get_points(adjust_data* record, int hitbox, bool from_aim);
