...
void knifebot::scan_targets()
{
	...
	for (auto& target : aim::get().targets)
	{
		if (target.history_record->valid())
		{
			...
			{
				auto last_distance = globals.g.eye_pos.DistTo(target.last_record->origin);
				auto history_distance = globals.g.eye_pos.DistTo(target.history_record->origin);
...
int GetMinimalHp() {
	if (TICKS_TO_TIME(globals.g.fixed_tickbase) > (globals.g.weapon->m_flNextPrimaryAttack() + 0.4f))
		return 34;
...
void knifebot::fire(CUserCmd* cmd)
{
	...
	auto vecDelta = vecMins - globals.g.eye_pos;

	if (vecDelta.Length() > 60.0f)
		return;

	vecDelta.Normalize();
	auto delta = fabs(math::normalize_yaw(final_target.record->angles.y - math::calculate_angle(final_target.record->player->get_shoot_position(), globals.local()->GetAbsOrigin()).y));
...
int knifebot::determinate_hit_type(bool stab_type, const Vector& delta) 
{
	...
	auto end = globals.g.eye_pos + delta * minimum_distance;
	...
	ray.Init(globals.g.eye_pos, end, Vector(-16.0f, -16.0f, -18.0f), Vector(16.0f, 16.0f, 18.0f));
	...
	auto final_delta = final_target.record->origin - globals.g.eye_pos;
