...
void antiaim::create_move(CUserCmd* m_pcmd)
{
	...
	if (g_cfg.antiaim.antiaim_type)
		type = ANTIAIM_LEGIT;
	else if (velocity >= 5.0f && globals.g.slowwalking)
		type = ANTIAIM_SLOW_WALK;
	else if (velocity >= 5.0f && globals.local()->m_fFlags() & FL_ONGROUND && engineprediction::get().backup_data.flags & FL_ONGROUND)
		type = ANTIAIM_MOVE;
	else if (!(globals.local()->m_fFlags() & FL_ONGROUND && engineprediction::get().backup_data.flags & FL_ONGROUND))
		type = ANTIAIM_AIR;
...
float antiaim::get_yaw(CUserCmd* m_pcmd)
{
	...
	{	
		...
		else if (g_cfg.antiaim.type[type].desync == 2)
			flip = invert_jitter;
	...
	else
	{
		...
		auto base_angle = 0.0f;

		auto ee_yaw = 0.f;
		if ((m_pcmd->m_buttons & IN_USE))
			ee_yaw = 180.f + m_pcmd->m_viewangles.y;
		else
			ee_yaw = 180.f + m_pcmd->m_viewangles.y;
		...
		if (g_cfg.antiaim.type[type].base_angle == 1 && manual_side == SIDE_NONE)
			base_angle = at_targets();
		else if (g_cfg.antiaim.type[type].base_angle == 0)
			base_angle = ee_yaw;
		...
		yaw_angle = invert_jitter ? (float)g_cfg.antiaim.type[type].range_jitter * -0.5f : (float)g_cfg.antiaim.type[type].range_jitter * 0.5f;
		...
		if (g_cfg.antiaim.type[type].desync)
		{
			...
			if (type == ANTIAIM_STAND && g_cfg.antiaim.type[type].lby_type)
				desync_delta *= 2.0f;
			else
			{
				if (!flip)
			...
			if (type != ANTIAIM_STAND && type != ANTIAIM_LEGIT || !g_cfg.antiaim.type[type].lby_type)
			{
				if (!flip)
		...
		lby_type = g_cfg.antiaim.type[type].lby_type;
	}
