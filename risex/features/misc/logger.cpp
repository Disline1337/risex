...
namespace notify
{
	void render(ImDrawList* draw)
	{
		...
		for (size_t i = 0; i < notifications.size(); i++)
		{
			...
			ImVec2 textSize = c_menu::get().futura_small->CalcTextSizeA(14.f, FLT_MAX, 0.0f, notify.type.c_str());
			...
			ImVec2 all_textSize = c_menu::get().futura_small->CalcTextSizeA(14.f, FLT_MAX, 0.0f, all_text.c_str());
	...
	namespace console {
		...
		void notify(std::string pre, std::string msg, Color clr_pre) {
			m_cvar()->ConsoleColorPrintf(g_cfg.menu.menu_theme, "[risex] ");
