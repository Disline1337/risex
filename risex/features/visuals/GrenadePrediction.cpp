...
void GrenadePrediction::Paint()
{
	...
	if (target.second && weapon->m_iItemDefinitionIndex() == WEAPON_HEGRENADE && (!g_cfg.esp.on_click || globals.get_command()->m_buttons & IN_ATTACK || globals.get_command()->m_buttons & IN_ATTACK2)) {
		Vector screen;
		...
		if (math::world_to_screen(prev, screen))
			render::get().text(fonts[INDICATORFONT], screen.x - 4, screen.y - 28, (int)target.first >= target.second->m_iHealth() ? Color::Red : Color::White, HFONT_CENTERED_X | HFONT_CENTERED_Y, std::to_string((int)target.first).c_str());
	}
	...
	if (type && path.size() > 3)
	{
		...
		/*for (auto it = others.begin(); it != others.end(); ++it)
		{
			...
			if (math::world_to_screen(it->first, screen))
				render::get().rect_filled(screen.x - 2, screen.y - 2, 4, 4, it->second == Color::Green ? Color::Green : g_cfg.esp.grenade_prediction_color);
		}*/
		...
		if (math::world_to_screen(prev, nadeEnd))
			render::get().rect_filled(nadeEnd.x - 2, nadeEnd.y - 2, 4, 4, g_cfg.esp.grenade_prediction_color);

		Vector endpos = path[path.size() - 1];

		if (weapon->m_iItemDefinitionIndex() == WEAPON_SMOKEGRENADE)
		{
			render::get().Circle3D(endpos, 144, g_cfg.esp.grenade_prediction_tracer_color);
		...
		{
			render::get().Circle3D(endpos, 150, g_cfg.esp.grenade_prediction_tracer_color);
		...
		{
			render::get().Circle3D(endpos, 148, g_cfg.esp.grenade_prediction_tracer_color);
			//render::get().Draw3DFilledCircle(endpos, 148, g_cfg.esp.grenade_prediction_tracer_color);
		}
		else if (weapon->m_iItemDefinitionIndex() == WEAPON_FLASHBANG)
		{
			render::get().Circle3D(endpos, 20, g_cfg.esp.grenade_prediction_tracer_color);
			//render::get().Draw3DFilledCircle(endpos, 20, g_cfg.esp.grenade_prediction_tracer_color);
		}
