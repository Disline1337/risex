...
void nightmode::modulate(MaterialHandle_t i, IMaterial* material, bool backup = false) 
{
	...
	if (strstr(name, crypt_str("World")))
	{
		...
		material->AlphaModulate((float)1.f);
	...
	else if (strstr(name, crypt_str("StaticProp")))
	{
		...
		material->ColorModulate(((float)g_cfg.esp.world_color.b() + 20) / 255.0f, ((float)g_cfg.esp.world_color.b() + 20) / 255.0f, ((float)g_cfg.esp.world_color.b() + 20) / 255.0f);
	}
}
