...
namespace math
{
	...
	bool normalize_layer(float layer, float min, float max)
	{
	...
	float skeet_math(float destAngle, float srcAngle, float lastAngle) {
		float delta;

		delta = fmodf(destAngle - srcAngle + lastAngle, 360.0f);
		if (destAngle > srcAngle) {
			if (delta >= 180)
				delta -= 360;
		}
		...
		if (lastAngle)
		{
			...
			delta = DEG2RAD(destAngle / srcAngle + lastAngle);
	...
	QAngle Interpolate(const QAngle from, const QAngle to, const float percent) {
