...
void __fastcall hooks::hooked_runcommand(void* ecx, void* edx, player_t* player, CUserCmd* m_pcmd, IMoveHelper* move_helper)
{
	...
	if (!player || player != globals.local())
	...
	if (g_cfg.ragebot.enable && player->is_alive())
	{
		...
		player->m_flVelocityModifier() = globals.g.last_velocity_modifier;
...
void __vectorcall hooks::hooked_clmove(float accumulated_extra_samples, bool bFinalTick)
{
	if (globals.g.fakeducking)
		return (cl_move_t(hooks::original_clmove)(accumulated_extra_samples, bFinalTick));

	if (globals.g.startcharge && globals.g.tocharge < globals.g.tochargeamount)
	{
		globals.g.tocharge++;
		globals.g.ticks_allowed = globals.g.tocharge;
	...
	while (globals.g.shift_ticks)
	{
		globals.g.isshifting = true;
		globals.g.shift_ticks--;
		globals.g.tocharge--;
		(cl_move_t(hooks::original_clmove)(accumulated_extra_samples, bFinalTick));
	}
...
bool __fastcall hooks::hooked_writeusercmddeltatobuffer(void* ecx, void* edx, int slot, bf_write* buf, int from, int to, bool is_new_command)
{
	...
	*new_commands = choked_modifier;
	...
	auto tickbase_s = globals.local()->m_nTickBase();

	int32_t loss = min(tickbase_s, 14);

	tickbase_s -= loss;
	m_clientstate()->pNetChannel->m_nOutSequenceNr += loss;
next_cmd:
