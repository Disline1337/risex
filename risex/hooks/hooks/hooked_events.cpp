...
void C_HookedEvents::FireGameEvent(IGameEvent* event)
{
	...
	else if (!strcmp(event_name, crypt_str("bullet_impact")))
	{
		...
		if (user == m_engine()->GetLocalPlayer())
		{
			...
			if (g_cfg.esp.server_bullet_impacts)
				m_debugoverlay()->BoxOverlay(position, Vector(-1.5f, -1.5f, -1.5f), Vector(1.5f, 1.5f, 1.5f), QAngle(0.0f, 0.0f, 0.0f), g_cfg.esp.server_bullet_impacts_color.r(), g_cfg.esp.server_bullet_impacts_color.g(), g_cfg.esp.server_bullet_impacts_color.b(), g_cfg.esp.server_bullet_impacts_color.a(), 4.0f);
	...
	else if (!strcmp(event_name, crypt_str("player_hurt")))
	{
		...
		if (attacker_id == m_engine()->GetLocalPlayer() && user_id != m_engine()->GetLocalPlayer())
		{
			...
			if (current_shot)
			{
				...
				hitmarker::get().add_hit(hitmarker_t(m_globals()->m_curtime, user_id, damage, hitgroup, entity->hitbox_position_matrix(get_hitbox_by_hitgroup(hitgroup), current_shot && entity == aim::get().last_target[current_shot->last_target].record.player ? aim::get().last_target[current_shot->last_target].record.matrixes_data.main : entity->m_CachedBoneData().Base())));
	...
	{
		if (!g_cfg.misc.show_default_log && eventlogs::get().last_log && g_cfg.misc.log_output)
		{
		...
		for (auto i = 1; i < m_globals()->m_maxclients; i++)
		{
			...
			c_anim_fix::get().player_resolver[i].Reset();
