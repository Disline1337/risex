...
namespace hooks
{
	...
	void GUI_Init(IDirect3DDevice9* pDevice)
	{
		...
		auto verdana_directory = (std::string)windows_directory + "\\Fonts\\Small Fonts.ttf";

		// Menu fonts
		c_menu::get().futura_small = io.Fonts->AddFontFromMemoryTTF((void*)MontserratRegular, sizeof(MontserratRegular), 15.f, &m_config, ranges);
		c_menu::get().futura_smallest = io.Fonts->AddFontFromMemoryTTF((void*)MontserratRegular, sizeof(MontserratRegular), 12.f, &m_config, ranges);
		c_menu::get().futura = io.Fonts->AddFontFromMemoryTTF((void*)MontserratRegular, sizeof(MontserratRegular), 17.f, &m_config, ranges);
		c_menu::get().futura_large = io.Fonts->AddFontFromMemoryTTF((void*)arialbolditalicmt, sizeof(arialbolditalicmt), 20.f, &m_config, ranges);
		c_menu::get().gotham = io.Fonts->AddFontFromMemoryTTF((void*)GothamPro, sizeof(GothamPro), 13.f, &m_config, ranges);
		c_menu::get().astrium = io.Fonts->AddFontFromMemoryTTF((void*)astr, sizeof(astr), 15.f, &m_config, io.Fonts->GetGlyphRangesCyrillic());
	...
	long __stdcall hooked_present(IDirect3DDevice9* device, RECT* src_rect, RECT* dest_rect, HWND dest_wnd_override, RGNDATA* dirty_region)
	{
		...
		c_menu::get().keys();
		c_menu::get().watermark();
		c_menu::get().spectators();
		worldesp::get().spread_crosshair(device);
