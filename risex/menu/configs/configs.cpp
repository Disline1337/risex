...
void C_ConfigManager::setup()
{
	...
	setup_item(&g_cfg.ragebot.zeus_bot, true, crypt_str("Ragebot.zeus_bot"));
	setup_item(&g_cfg.ragebot.knife_bot, true, crypt_str("Ragebot.knife_bot"));
	setup_item(&g_cfg.ragebot.autoshoot, false, crypt_str("Ragebot.autoshoot"));
	setup_item(&g_cfg.ragebot.double_tap, true, crypt_str("Ragebot.double_tap"));
	...
	setup_item(&g_cfg.antiaim.hide_shots, true, crypt_str("Ragebot.hide_shots"));
	...
	for (auto i = 0; i < 4; i++)
	{
		setup_item(&g_cfg.antiaim.type[i].desync, 0, crypt_str("Antiaim.desync"));
		setup_item(&g_cfg.antiaim.type[i].lby_type, 0, crypt_str("Antiaim.lby_type"));
		setup_item(&g_cfg.antiaim.type[i].range_jitter, 0, std::to_string(i) + crypt_str("Antiaim.jitter_range"));
	...
	setup_item(&g_cfg.player.lag_hitbox_chams, 0, crypt_str("Antiaim.lag_hitbox_chams"));
	...
	setup_item(&g_cfg.esp.hitmarker, false, crypt_str("Esp.hit_marker"));
	setup_item(&g_cfg.esp.grenade_tracer_color, Color(255, 255, 255), crypt_str("Esp.grenade_tracer_color"));
	...
	setup_item(&g_cfg.esp.grenade_fff, false, crypt_str("Esp.grenade_fff"));
	setup_item(&g_cfg.esp.grenade_proximity_tracers_mode, 0, crypt_str("Esp.grenade_proximity_tracers_mode"));
	setup_item(&g_cfg.esp.proximity_tracers_width, 0, crypt_str("Esp.proximity_tracers_width"));
	...
	setup_item(&g_cfg.esp.grenade_color_warning, Color(255, 255, 255), crypt_str("Esp.grenade_color_warning"));
	...
	setup_item(&g_cfg.menu.spectators_list, false, crypt_str("Menu.spectators_list"));
...
{
	std::string file = crypt_str("C:\\Risex\\Configs\\CSGO\\") + config;
	json allJson;
...
{
	std::string file = crypt_str("C:\\Risex\\Configs\\CSGO\\");

	files.clear();

	std::string path = file + crypt_str("/*.rx");
	WIN32_FIND_DATA fd;
