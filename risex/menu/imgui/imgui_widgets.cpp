...
bool ImGui::TabEx(const char* icon, const char* label, const bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImVec2 size = CalcItemSize(size_arg, 50 + style.FramePadding.x, label_size.y + style.FramePadding.y);
    ...
    static float size_c = 2.f;
    if (active && sizing == 4.f)
    {
        if (size_c < 9.f)
            size_c += 0.2f;
        else
    ...
    if (held || hovered && !active && alpha < 0.95f && sizing == 4.f)
    {
        alpha += 0.02f;
    }
    ...
    if (active)
    {
        PushFont(c_menu::get().ico_bottom);
        Render->AddText({ bb.Min - style.FramePadding - ImVec2(0, 3) }, ImColor(g_cfg.menu.menu_theme.r(), g_cfg.menu.menu_theme.g(), g_cfg.menu.menu_theme.b(), int(c_menu::get().menu_alpha * 255)), icon);
        ImGui::PopFont();
        Render->AddText({ bb.Max.x - label_size.x - 10, bb.Min.y }, ImColor(220, 220, 220, int(c_menu::get().menu_alpha * 255)), label);

        // GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Min.x, bb.Min.y + sized), ImGui::GetColorU32(ImVec4(0.09f, 0.1f, 0.11f, alpha)));
    }
    else if (hovered)
    {
        PushFont(c_menu::get().ico_bottom);
        Render->AddText({ bb.Min - style.FramePadding - ImVec2(0, 3) }, ImColor(170, 170, 170, int(c_menu::get().menu_alpha * 255)), icon);
        ImGui::PopFont();
        Render->AddText({ bb.Max.x - label_size.x - 10, bb.Min.y }, ImColor(240, 240, 240, int(c_menu::get().menu_alpha * 255)), label);
    }
    else if (hovered && active)
    {
        PushFont(c_menu::get().ico_bottom);
        Render->AddText({ bb.Min - style.FramePadding - ImVec2(0, 3) }, ImColor(160, 160, 160, int(c_menu::get().menu_alpha * 255)), icon);
        ImGui::PopFont();
        Render->AddText({ bb.Max.x - label_size.x - 10, bb.Min.y }, ImColor(230, 230, 230, int(c_menu::get().menu_alpha * 255)), label);
    }
    else
    {
        PushFont(c_menu::get().ico_bottom);
        Render->AddText({ bb.Min - style.FramePadding - ImVec2(0, 3) }, ImColor(130, 130, 130, int(c_menu::get().menu_alpha * 255)), icon);
        ImGui::PopFont();
        Render->AddText({ bb.Max.x - label_size.x - 10, bb.Min.y }, ImColor(160, 160, 160, int(c_menu::get().menu_alpha * 255)), label);
    }
...
bool ImGui::SubTabEx(const char* label, const char* idz, const bool active, const ImVec2& size_arg)
{
...
bool ImGui::SubTab(const char* label, const char* id, const bool active, const ImVec2& size_arg)
{
    return SubTabEx(label, id, active, size_arg);
}
...
bool ImGui::KeybindButton(const char* label, const char* unique_id, const ImVec2& size_arg, bool clicked, ImGuiButtonFlags flags)
{
    ...
    PushFont(c_menu::get().futura_small);
...
bool ImGui::VisualsAdder(const char* label, const char* unique_id, const ImVec2& size_arg, bool clicked, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderFrame(bb.Min, bb.Min + ImVec2(label_size.x * 2, 0), ImColor(24, 22, 29), true, 2.f * c_menu::get().dpi_scale);
    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Min + ImVec2(label_size.x * 2, 0), GetColorU32(ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, it_hover->second)), 2.f * c_menu::get().dpi_scale);
    PushFont(c_menu::get().futura_small);
    PushStyleColor(ImGuiCol_Text, it_hcolor->second);
    RenderText(bb.Min + style.FramePadding, label);
    PopStyleColor();
    PopFont();
...
bool ImGui::CustomButton(const char* label, const char* label_id, const ImVec2& size_arg, bool use_icon, ImFont* icon_font, const char* icon_label)
{
    ...
    ImGui::GetWindowDrawList()->AddCircleFilled(it_circle->second.pos, it_circle->second.radius, GetColorU32(ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, 1.f)), 60);
    window->DrawList->PopClipRect();
    if (hovered) {

        RenderFrame(bb.Min, bb.Max, ImColor(69 / 255.f, 71 / 255.f, 90 / 255.f, it_hover->second - (55 / 255.f)), true, 2.f);
        ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, ImColor(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, it_hover->second - (30 / 255.f)), 2.f);
    }
    if (use_icon)
    {
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    window->DrawList->AddRectFilled(ImVec2(grab_rect.Max.x - 3, grab_rect.Min.y), grab_rect.Max, grab_col, 2);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    RenderFrame(check_bb.Min, check_bb.Max, ImColor(33.f / 255.f, 33.f / 255.f, 33.f / 255.f, 1.f), true, 0);
    window->DrawList->AddRect(check_bb.Min, check_bb.Max, ImColor(20, 20, 20, 225), 2, 15, 1);
    ImColor mm = ImColor(255, 255, 255);
    if (*v) {
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImColor(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, it_filled->second), 2);
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImColor(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, it_filled->second - 0.85f), 2);
        const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 5.f));
        RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), mm, square_sz - pad * 2.0f);
    }

    if (hovered)
        window->DrawList->AddRect(check_bb.Min, check_bb.Max, ImColor(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, it_filled->second), 2);
    //ImGui::PushStyleColor(ImGuiCol_Text, it_text->second);

    if (label_size.x > 0.0f)
        window->DrawList->AddText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + 2 * c_menu::get().dpi_scale - (1 * (1.8 - c_menu::get().dpi_scale))), *v ? ImColor(255, 255, 255, int(it_text->second.z * 255)) : ImColor(200, 200, 200, 250), label);
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        if (GroupStack.Size > 0 && WindowID == window->ID)
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int* current_item)
{
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(value_x2, frame_bb.Max.y), ImColor(33, 33, 33), 1, ImDrawCornerFlags_Left);

    window->DrawList->AddRectFilledMultiColor(ImVec2(value_x2, frame_bb.Min.y), frame_bb.Max, ImColor(g_cfg.menu.menu_theme.r(), g_cfg.menu.menu_theme.g(), g_cfg.menu.menu_theme.b(), 255), ImColor(g_cfg.menu.menu_theme.r(), g_cfg.menu.menu_theme.g(), g_cfg.menu.menu_theme.b(), 255), ImColor(g_cfg.menu.menu_theme.r() - 30, g_cfg.menu.menu_theme.g() - 30, g_cfg.menu.menu_theme.b() - 30, 255), ImColor(g_cfg.menu.menu_theme.r() - 30, g_cfg.menu.menu_theme.g() - 30, g_cfg.menu.menu_theme.b() - 30, 255));

    if (flags & ImGuiComboFlags_NoArrowButton)
    {
        window->DrawList->AddLine(ImVec2(value_x2 + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y + 4), ImVec2(frame_bb.Max.x - 4, frame_bb.Min.y + style.FramePadding.y + 4), ImColor(20, 20, 20, 200));
        window->DrawList->AddLine(ImVec2(value_x2 + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y + 7), ImVec2(frame_bb.Max.x - 4, frame_bb.Min.y + style.FramePadding.y + 7), ImColor(20, 20, 20, 255));
        window->DrawList->AddLine(ImVec2(value_x2 + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y + 10), ImVec2(frame_bb.Max.x - 4, frame_bb.Min.y + style.FramePadding.y + 10), ImColor(20, 20, 20, 200));
    }
    else
        RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y), ImColor(20, 20, 20, 255), it_arrow_alpha->second.dir, 1.0f);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, bool minimum_damage, float power)
{
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(1.5f, 1.5f) - ImVec2(0, 2.5f), grab_bb.Max + ImVec2(0.2f, 2.2f) + ImVec2(0, 0), ImColor(g_cfg.menu.menu_theme.r(), g_cfg.menu.menu_theme.g(), g_cfg.menu.menu_theme.b(), 255), 0.0f);
    ...
    window->DrawList->AddText(ImVec2(frame_bb.Max.x - CalcTextSize(value_buf).x, frame_bb.Min.y - CalcTextSize(value_buf).y - (4 * c_menu::get().dpi_scale)), (*(int*)p_data > 1) ? ImColor(255, 255, 255) : ImColor(170, 170, 170, 200), value_buf, value_buf_end);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    {
        RenderNavHighlight(frame_bb, id);
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(20, 20, 20), 1);
        window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Max.y - 2), ImVec2(frame_bb.Max.x, frame_bb.Max.y - 2), ImColor(200, 200, 200, 200), 1);
    }
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    SetCursorPosY(GetCursorPosY() - 2 * 1);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        draw_list->AddRect(ImVec2(bar0_pos_x + 1, bar0_line_y - 1), ImVec2(bar0_pos_x + bars_width, bar0_line_y + 1), ImColor(20, 20, 20, 255));
        //RenderArrowsForVerticalBar(draw_list, ImVec2(bar0_pos_x - 1, bar0_line_y), ImVec2(bars_triangles_half_sz + 1, bars_triangles_half_sz), bars_width + 2.0f);
    }
    ImColor color_for_grip;
    float max_white[3] = { 200,200,200 };
    float max_Back[3] = { 30,30,30 };
    if (col >= max_white)
        color_for_grip = { 20,20,20 };
    else  if (col <= max_Back)
        color_for_grip = { 210,210,210 };
    static float radius_grap = 3;
    if (value_changed) {
        if (radius_grap < 6.f)
            radius_grap += 0.1f;
        else
    ...
    draw_list->AddCircle(sv_cursor_pos - ImVec2(1, 1), radius_grap + 1, ImColor(20, 20, 20), 16, 1);
    draw_list->AddCircle(sv_cursor_pos - ImVec2(1, 1), radius_grap, ImColor(200, 200, 200), 16, 1);
    draw_list->AddCircle(sv_cursor_pos - ImVec2(1, 1), radius_grap - 1, ImColor(20, 20, 20), 16, 1);
    ...
    if (alpha_bar)
    {
        ...
        draw_list->AddRect(bar1_bb.Min, bar1_bb.Max, ImColor(0, 0, 0, 255));
        // window->DrawList->AddRectFilled(bar1_bb.Min, bar1_bb.Max, ImColor(0, 0, 0, 255), 0); // Color button are often in need of some sort of border
         //RenderColorRectWithAlphaCheckerboard(draw_list, bar1_bb.Min, bar1_bb.Max, 0, bar1_bb.GetWidth() / 2.0f, ImVec2(0.0f, 0.0f));
        // window->DrawList->AddRectFilled(bar1_bb.Min, bar1_bb.Max, ImColor(0, 0, 0, 230), 0); // Color button are often in need of some sort of border
        draw_list->AddRectFilled(bar1_bb.Min, bar1_bb.Max, ImColor(R, G, B, AL));
        ...
        draw_list->AddRect(ImVec2(bar1_pos_x + 1, bar1_line_y - 1), ImVec2(bar1_pos_x + bars_width, bar1_line_y + 1), ImColor(20, 20, 20, 255));
    }
    ...
    if (CustomButton("Copy", "##COPY_COLOR", ImVec2(square_sz * 5.77f, 0)))
    {

        memcpy(c_menu::get().color_buffer, col, components * sizeof(float));
    }
    SameLine();
    if (CustomButton("Paste", "##COPY_PASTE", ImVec2(square_sz * 5.77f, 0)))
    {
    ...
    if (rainbow_color) {
        ...
        {
            float flvalue = (1.f / (float)1337) * i;
            flvalue -= rainbow;
            if (flvalue < 0.f)
                flvalue += 1.f;
            ImColor().HSV(flvalue, 1.f, 1.f);
            ColorConvertHSVtoRGB(flvalue, 1.f, 1.f, col[0], col[1], col[2]);
        }
        value_changed = true;
    }
    return value_changed;
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    ImVec4 col_rgb_with_alpha(col_rgb.x, col_rgb.y, col_rgb.z, col_rgb.w);
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f)
    {
        ...
        window->DrawList->AddRect(bb_inner.Min, ImVec2(mid_x, bb_inner.Max.y), ImColor(0, 0, 0, 255), 0); // Color button are often in need of some sort of border
        window->DrawList->AddRectFilledMultiColor(bb_inner.Min, ImVec2(mid_x, bb_inner.Max.y), GetColorU32(col_rgb_without_alpha), GetColorU32(col_rgb_without_alpha), GetColorU32(col_rgb_with_alpha), GetColorU32(col_rgb_with_alpha));
    }
    else
    {
        float mid_x = IM_ROUND((bb_inner.Min.x + bb_inner.Max.x) * 0.5f);
        ...
        window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, ImColor(0, 0, 0, 255), 0); // Color button are often in need of some sort of border
        RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min, bb_inner.Max, GetColorU32(col_rgb), grid_step, ImVec2(-grid_step + off, off), rounding, ImDrawCornerFlags_TopRight | ImDrawCornerFlags_BotRight);
        window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, ImColor(0, 0, 0, 230), 0); // Color button are often in need of some sort of border
        window->DrawList->AddRectFilledMultiColor(bb_inner.Min, bb_inner.Max, GetColorU32(col_rgb_without_alpha), GetColorU32(col_rgb_without_alpha), GetColorU32(col_rgb_with_alpha), GetColorU32(col_rgb_with_alpha));
    }
    RenderNavHighlight(bb, id);
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
        if (g.Style.FrameBorderSize > 0.0f)
            RenderFrameBorder(bb.Min, bb.Max, 3);
        else

            window->DrawList->AddRect(bb.Min, bb.Max, ImColor(33, 32, 33, 255), 3); // Color button are often in need of some sort of border
    }
    if (hovered)
        window->DrawList->AddRect(bb.Min, bb.Max, ImColor(g_cfg.menu.menu_theme.r(), g_cfg.menu.menu_theme.g(), g_cfg.menu.menu_theme.b(), 255), 3); // Color button are often in need of some sort of border
    ...
    if (g.ActiveId == id && !(flags & ImGuiColorEditFlags_NoDragDrop) && BeginDragDropSource())
    {
        ...
        SameLine();
        TextEx("You paste this color:");
        SameLine(135);
        ColorButton(ImGuiColorEditFlags_NoAlpha ? IMGUI_PAYLOAD_TYPE_COLOR_4F : IMGUI_PAYLOAD_TYPE_COLOR_3F, col_rgb, flags);
        EndDragDropSource();
    }
...
bool ImGui::CustomSelectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    const ImVec4 text_dis = ImVec4(202 / 255.f, 202 / 255.f, 202 / 255.f, 1.f);
    float deltatime = 1.8f * ImGui::GetIO().DeltaTime;
